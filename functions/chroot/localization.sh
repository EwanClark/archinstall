...
timezone() {
  ...
  read -r -p "Press enter to see all available timezones"
  timedatectl list-timezones
  local timezone_input=""
  local resolved_timezone=""
  while true; do
    read -r -p "Enter your timezone (e.g. Region/City): " timezone_input
...
locale() {
  read -r -p "Would you like a custom locale (default is en_us) [Y/n]: " custom_locale
  custom_locale="${custom_locale,,}"
  if [[ -z "$custom_locale" || "$custom_locale" == "y" || "$custom_locale" == "yes" ]]; then
    read -r -p "Press enter to be taken to the file to uncomment the locale you want to use"
    nano /etc/locale.gen
    read -r -p "Have you uncommented the locale you want to use? [Y/n]: " uncommented_locale
...
hostname() {
  ...
  while true; do
    read -r -p "Enter hostname (default is arch): " hostname_input
